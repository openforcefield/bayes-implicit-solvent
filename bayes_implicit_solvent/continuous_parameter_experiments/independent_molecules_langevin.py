...
try:
    ...
    job_id = np.random.randint(len(smiles_list)) + 1
    print("No valid job_id supplied! Selecting one at random: ", job_id)

smiles_subset = [smiles_list[job_id - 1]]
...
n_configuration_samples = 20  # TODO: Since this is cheaper, can probably modify this a bit...
...
from bayes_implicit_solvent.continuous_parameter_experiments.elemental_types_mh import sample_path_to_unitted_snapshots, paths_to_samples
...
for path in paths_to_samples:
    ...
    print('thinned vacuum_traj from {} to {}'.format(len(vacuum_samples), len(mol.vacuum_traj)))
...
if __name__ == '__main__':
    ...
    traj, log_probs, grads, acceptance_probabilities, stepsizes = MALA(
        x0=theta1, stepsize=1e-4, log_prob_fun=log_prob, grad_log_prob_fun=grad_log_prob, n_steps=n_steps, adapt_stepsize=True)

    np.savez(os.path.join(data_path,
                          'all_types_independent_mala_freesolv_{}.npz'.format(
                              name)),
             traj=traj, grads=grads, log_probs=log_probs, acceptance_probabilities=acceptance_probabilities, stepsizes=stepsizes)
