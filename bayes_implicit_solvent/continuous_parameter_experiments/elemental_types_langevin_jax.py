...
# UPDATE: It works, but it's way slower than
...
@jit
def log_likelihood_component(theta, i):
    prediction = predict_solvation_free_energy_jax(theta, distance_matrices[i], charges[i], element_inds[i])
    return - ((prediction - expt_means[i])**2 / (expt_uncs[i]**2))

@jit
def log_likelihood(theta):
    return np.sum([log_likelihood_component(theta, i) for i in range(len(mols))])

@jit
def grad_log_likelihood_component(theta, i):
    return grad(lambda theta : log_likelihood_component(theta, i))(theta)

@jit
def grad_log_likelihood(theta):
    return np.sum([grad_log_likelihood_component(theta, i) for i in range(len(mols))], axis=0)
...
@jit
def grad_log_prob_fun(theta):
    return grad_log_prior(theta) + grad_log_likelihood(theta)

if __name__ == '__main__':

    from bayes_implicit_solvent.samplers import random_walk_mh, MALA, langevin
    x0 = np.array(prior_location)
    v0 = np.random.randn(len(prior_location)) * 0.01
    ...
    np.savez('freesolv_langevin_jax_big_collision_rate.npz',
             traj=traj,
