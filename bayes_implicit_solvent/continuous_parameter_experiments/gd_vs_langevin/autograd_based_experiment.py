...
# write an integer in range(0, n) on each atom indicating which type it was assigned
mbondi_type_slices = [mbondi_model.apply_to_molecule(mol.mol) for mol in molecules]

def unpack(theta):
    """Given a flat parameter array theta of length 2n,
    unpack it into two arrays of length n,
...
def assign_radii_and_scales(theta, type_slices):
    ...
    Parameters
    ----------
    theta : array of length 2n
    type_slices : list of variable-length integer arrays taking values in range(0,n)
...
def generate_all_predictions(theta):
    ...
    predictions = np.array([molecules[i].predict_solvation_free_energy_autograd(radii, scales) for i, (radii, scales) in enumerate(parameterized_list)])
    return predictions
